...
interface WeeklyReviewDialogProps {
  ...
  tasks: Task[];
  onSaveReview: (updates: Partial<Omit<WeeklyReview, 'id' | 'createdAt' | 'updatedAt' | 'weekStart'>>) => Promise<WeeklyReview | null>;
  weeklyStats: {
    totalCreated: number;
...
export function WeeklyReviewDialog({
  ...
  const [reviewedTasks, setReviewedTasks] = useState<Set<string>>(new Set(currentReview?.incompleteTasksReviewed || []));
            ...
            <div className="space-y-6 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-lg bg-primary/10 text-center">
                  <span className="text-3xl font-bold text-primary">{weeklyStats.completedThisWeek}</span>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <div className="flex-1 p-2 rounded bg-destructive/10 text-center">
                    <span className="font-semibold text-destructive">{weeklyStats.byPriority.high}</span>
                    <p className="text-xs text-muted-foreground">High</p>
                  </div>
                  <div className="flex-1 p-2 rounded bg-warning/10 text-center">
                    <span className="font-semibold text-warning">{weeklyStats.byPriority.medium}</span>
          ...
          {step === 'incomplete' && (
            <div className="space-y-4 py-4">
              <div className="text-center mb-4">
                ...
                <p className="text-muted-foreground text-sm">Check tasks you've reviewed and have a plan for</p>
              </div>
          ...
          {step === 'intentions' && (
            <div className="space-y-4 py-4">
              <div className="text-center mb-4">
                ...
                <p className="text-muted-foreground text-sm">What do you want to accomplish next week?</p>
              </div>
          ...
          {step === 'celebrate' && (
            <div className="space-y-4 py-4">
              <div className="text-center mb-4">
                ...
                <p className="text-muted-foreground text-sm">What went well this week? Celebrate your accomplishments!</p>
              </div>
              ...
              {completedTasks.length > 0 && (
                <div className="p-3 rounded-lg bg-primary/10 border border-primary/20">
                  <p className="text-sm font-medium text-primary">
                    🎉 You completed {completedTasks.length} task{completedTasks.length !== 1 ? 's' : ''} this week!
                  </p>
                </div>
              )}
            </div>
