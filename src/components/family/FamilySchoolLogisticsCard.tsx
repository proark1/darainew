...
export function FamilySchoolLogisticsCard() {
  const {
    upcomingSchoolEvents, todaysRota, rota, classmates,
    equipmentNeedingReplacement, isLoading,
  } = useFamilySchool();
          ...
          {isLoading ? (
            ...
            <>
              ...
              <section className="space-y-2">
                <div className="flex items-center justify-between">
                  <h4 className="text-sm font-medium flex items-center gap-1.5">
                    <GraduationCap className="h-4 w-4" /> School Events ({upcomingSchoolEvents.length})
                  </h4>
                ...
                {upcomingSchoolEvents.slice(0, 3).map(e => {
                  ...
                  return (
                    <div key={e.id} className="flex items-center justify-between text-sm rounded-md bg-muted/50 px-2 py-1.5">
                      <div className="truncate">
                        <span className="font-medium">{e.title}</span>
                        <span className="text-xs text-muted-foreground ml-1.5">{memberName(e.family_member_id)} · {e.event_type}</span>
                      </div>
                ...
                {upcomingSchoolEvents.length === 0 && (
                  <p className="text-xs text-muted-foreground">No upcoming school events. Add term dates, exams, parent-teacher meetings.</p>
                )}
              </section>
              ...
              <section className="space-y-2">
                ...
                {todaysRota.length > 0 ? (
                  todaysRota.map(r => (
                    <div key={r.id} className="flex items-center justify-between text-sm rounded-md bg-muted/50 px-2 py-1.5">
                      <div className="truncate">
                        <span className="font-medium">{memberName(r.family_member_id)}</span>
                        {r.location && <span className="text-xs text-muted-foreground ml-1.5">· {r.location}</span>}
                      </div>
                      <div className="text-xs text-right">
                        {r.pickup_time && <div>↑ {r.pickup_time.slice(0, 5)}</div>}
                        {r.dropoff_time && <div>↓ {r.dropoff_time.slice(0, 5)}</div>}
                        {r.responsible_person && <div className="text-muted-foreground">{r.responsible_person}</div>}
                      </div>
                    </div>
                  ))
                ) : rota.length > 0 ? (
                  <p className="text-xs text-muted-foreground">Nothing scheduled for {DAYS[new Date().getDay()]}.</p>
                ) : (
                  <p className="text-xs text-muted-foreground">No rota set up. Add weekly pickup/dropoff schedule.</p>
                )}
              </section>
              ...
              <section className="space-y-2">
                ...
                {equipmentNeedingReplacement.slice(0, 3).map(e => (
                  <div key={e.id} className="flex items-center justify-between text-sm rounded-md bg-destructive/10 px-2 py-1.5">
                    <div className="truncate">
                      <span className="font-medium">{e.item_name}</span>
                      {e.size && <span className="text-xs text-muted-foreground ml-1.5">size {e.size}</span>}
                    </div>
                    <span className="text-xs text-muted-foreground">{memberName(e.family_member_id)}</span>
                  </div>
                ))}
                {equipmentNeedingReplacement.length === 0 && (
                  <p className="text-xs text-muted-foreground">No replacement needed. Track gear sizes for outgrown alerts.</p>
                )}
              </section>
              ...
              {classmates.length > 0 && (
                <section className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {classmates.slice(0, 4).map(c => c.child_name).join(' · ')}
                  </p>
                </section>
              )}
            </>
