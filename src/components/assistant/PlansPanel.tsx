...
import { usePlans, type PlanRow, type PlanStatus, type PlanStepStatus, type PlanStep } from '@/hooks/usePlans';
      ...
      <SheetContent className="w-full sm:max-w-md flex flex-col p-0">
        <SheetHeader className="px-4 py-3 border-b border-border">
          ...
          <p className="text-xs text-muted-foreground text-left">
            Multi-step plans the assistant proposed. Approve each step yourself; nothing runs without your click.
          </p>
        </SheetHeader>
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center text-center py-12 text-muted-foreground">
      ...
      <p className="text-xs mt-1 max-w-xs">
        Ask Dori for something multi-step ("plan my Dubai trip", "set up a weekly review")
...
function PlanCard(props: PlanCardProps) {
  ...
  const pct = plan.stepCount > 0
    ? Math.round((plan.completedStepCount / plan.stepCount) * 100)
    : 0;
    ...
    <div className="rounded-lg border border-border bg-card p-3 space-y-3 animate-fade-in">
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between text-[10px] text-muted-foreground">
          <span>{plan.completedStepCount} of {plan.stepCount} done</span>
          <span>{pct}%</span>
        </div>
      ...
      {!terminal && plan.currentStep && (
        <div className="rounded-md border border-dashed border-border/70 bg-muted/30 p-2">
          ...
          {plan.currentStep.description && plan.currentStep.description !== plan.currentStep.title && (
            <p className="text-xs text-muted-foreground mt-0.5 line-clamp-3">{plan.currentStep.description}</p>
          )}
        </div>
      )}
        ...
        <div className="flex flex-wrap gap-1.5">
          {plan.status === 'paused' ? (
            <Button size="sm" variant="default" className="h-7 text-xs gap-1.5"
              disabled={busy} onClick={props.onResume}>
              <RotateCcw className="w-3 h-3" /> Resume
            </Button>
          ) : (
            <Button size="sm" variant="default" className="h-7 text-xs gap-1.5"
              disabled={busy || !plan.currentStep} onClick={props.onExecuteNext}>
          ...
          <Button size="sm" variant="ghost" className="h-7 text-xs gap-1.5"
            disabled={busy || !plan.currentStep} onClick={props.onSkip}>
            <SkipForward className="w-3 h-3" /> Skip
          </Button>
          {plan.status !== 'paused' && (
            <Button size="sm" variant="ghost" className="h-7 text-xs gap-1.5"
              disabled={busy} onClick={props.onPause}>
              <Pause className="w-3 h-3" /> Pause
            </Button>
          )}
          <Button size="sm" variant="ghost" className="h-7 text-xs gap-1.5 text-destructive hover:text-destructive ml-auto"
            disabled={busy} onClick={props.onAbort}>
    ...
    <div className="flex items-start gap-2 rounded px-2 py-1.5 bg-background/50">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-xs font-medium truncate">
          <span className="text-muted-foreground">{step.idx + 1}/{totalSteps}.</span> {step.title}
        </p>
        {step.result_summary && (
        ...
        {step.error_message && (
          <p className="text-[10px] text-destructive line-clamp-2 mt-0.5">
            {step.error_message}
          </p>
        )}
      </div>
...
function StepStatusIcon({ status }: { status: PlanStepStatus }) {
  switch (status) {
    ...
    default:
      return <div className="w-3.5 h-3.5 rounded-full border border-muted-foreground/40 shrink-0 mt-0.5" />;
  }
}
