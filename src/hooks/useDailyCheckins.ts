...
export function useDailyCheckins() {
  ...
  const fetchCheckins = useCallback(async (days = 30) => {
    if (!user?.id) return;
    ...
    try {
      ...
      setTodayMorning(typedData.find(c => c.checkin_date === today && c.checkin_type === 'morning') || null);
      setTodayEvening(typedData.find(c => c.checkin_date === today && c.checkin_type === 'evening') || null);
    } catch (error) {
  ...
  const getAverageStats = useCallback((days = 7) => {
