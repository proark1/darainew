...
export function useFamilySchool() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [se, r, cm, eq, hw] = await Promise.all([
        supabase.from('family_school_calendar').select('*').eq('user_id', user.id).gte('start_date', new Date(Date.now() - 7 * 86400000).toISOString().slice(0, 10)).order('start_date'),
        supabase.from('family_pickup_rota').select('*').eq('user_id', user.id).eq('is_active', true).order('day_of_week'),
        supabase.from('family_classmates').select('*').eq('user_id', user.id).order('child_name'),
        supabase.from('family_equipment').select('*').eq('user_id', user.id).order('updated_at', { ascending: false }),
        supabase.from('family_homework_schedule').select('*').eq('user_id', user.id).order('day_of_week'),
      ]);
  ...
  const addSchoolEvent = async (e: Omit<SchoolEvent, 'id' | 'user_id'>) => {
    if (!user) return null;
    const { data, error } = await supabase.from('family_school_calendar')
      .insert({ ...e, user_id: user.id }).select().single();
    if (error) { toast.error('Failed to add'); return null; }
    setSchoolEvents(prev => [...prev, data as SchoolEvent].sort((a, b) => a.start_date.localeCompare(b.start_date)));
  ...
  const addRotaEntry = async (r: Omit<PickupRota, 'id' | 'user_id'>) => {
    if (!user) return null;
    const { data, error } = await supabase.from('family_pickup_rota')
      .insert({ ...r, user_id: user.id }).select().single();
    if (error) { toast.error('Failed to add'); return null; }
  ...
  const addClassmate = async (c: Omit<Classmate, 'id' | 'user_id'>) => {
    if (!user) return null;
    const { data, error } = await supabase.from('family_classmates')
  ...
  const addEquipment = async (e: Omit<Equipment, 'id' | 'user_id'>) => {
    if (!user) return null;
    const { data, error } = await supabase.from('family_equipment')
      .insert({ ...e, user_id: user.id }).select().single();
    if (error) { toast.error('Failed to add'); return null; }
  ...
  const flagEquipment = async (id: string, needs: boolean, reason?: string) => {
    const { error } = await supabase.from('family_equipment')
      .update({ needs_replacement: needs, replacement_reason: reason || null }).eq('id', id);
    if (error) { toast.error('Failed to update'); return; }
    setEquipment(prev => prev.map(e => e.id === id ? { ...e, needs_replacement: needs, replacement_reason: reason || null } : e));
  ...
  return {
    schoolEvents, upcomingSchoolEvents, rota, todaysRota, classmates, equipment, equipmentNeedingReplacement, homework,
    isLoading, addSchoolEvent, addRotaEntry, addClassmate, addEquipment, flagEquipment, refetch: fetchAll,
  };
}
