...
export function useUserLocationSettings() {
  ...
  const updateLocation = (city: string, country: string, latitude?: number, longitude?: number, timezone?: string) => {
    updateSettingsMutation.mutate({ city, country, latitude: latitude || null, longitude: longitude || null, timezone });
  };
