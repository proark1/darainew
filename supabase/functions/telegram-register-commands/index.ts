...
const BASE_REGISTRATIONS: CommandRegistration[] = [
  ...
  { label: 'private_de', commands: TELEGRAM_PRIVATE_COMMANDS_DE, scope: { type: 'all_private_chats' }, language_code: 'de' },
  { label: 'groups', commands: TELEGRAM_GROUP_COMMANDS, scope: { type: 'all_group_chats' } },
  { label: 'groups_de', commands: TELEGRAM_GROUP_COMMANDS_DE, scope: { type: 'all_group_chats' }, language_code: 'de' },
];
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const workspaceChatIds = Array.isArray(body?.workspaceChatIds)
      ? body.workspaceChatIds.map((id: unknown) => Number(id)).filter((id: number) => Number.isFinite(id))
      : [];
    const registrations: CommandRegistration[] = [
      ...BASE_REGISTRATIONS,
      ...workspaceChatIds.flatMap((chatId: number): CommandRegistration[] => [
        { label: `workspace_${chatId}`, commands: TELEGRAM_WORKSPACE_COMMANDS, scope: { type: 'chat', chat_id: chatId } },
        { label: `workspace_${chatId}_de`, commands: TELEGRAM_WORKSPACE_COMMANDS_DE, scope: { type: 'chat', chat_id: chatId }, language_code: 'de' },
      ]),
    ];
    ...
    for (const entry of registrations) {
      ...
      results.push({ label: entry.label, ok: r.ok, telegram: data, registered: entry.commands.length });
    }
