...
async function transcribeWithOpenAi(bytes: Uint8Array, mime: string): Promise<TelegramTranscriptionResult> {
  const apiKey = Deno.env.get('OPENAI_API_KEY');
  if (!apiKey) return { transcript: null, language: null, confidence: null, durationSeconds: null, provider: null, error: 'OPENAI_API_KEY not configured' };

  const fd = new FormData();
  fd.append('model', 'whisper-1');
  fd.append('file', new Blob([bytes as unknown as BlobPart], { type: mime }), `telegram-voice.${extensionForMime(mime)}`);
  ...
  if (!r.ok) {
    ...
    return { transcript: null, language: null, confidence: null, durationSeconds: null, provider: 'openai', error: message };
  }
  return {
...
async function transcribeWithGemini(bytes: Uint8Array, mime: string): Promise<TelegramTranscriptionResult> {
  const apiKey = Deno.env.get('GEMINI_API_KEY');
  if (!apiKey) return { transcript: null, language: null, confidence: null, durationSeconds: null, provider: null, error: 'GEMINI_API_KEY not configured' };
  ...
  if (!r.ok) {
    ...
    return { transcript: null, language: null, confidence: null, durationSeconds: null, provider: 'gemini', error: message };
  }
...
export async function transcribeTelegramVoice(args: {
  ...
  try {
    ...
    const errors: string[] = [];
    for (const provider of providers.split(',').map((p) => p.trim().toLowerCase()).filter(Boolean)) {
