...
export function decideTier(s: TierSignals, proModel: string | null): TierDecision {
  const flash: TierDecision = {
    model: FLASH_MODEL,
    maxRounds: FLASH_ROUNDS,
    tier: "flash",
    reason: "default",
  };
  ...
  if (s.routeConfidence > 0 && s.routeConfidence < 0.4 && msg.length > 40)
...
export async function runTriage(
  ...
  try {
    ...
    const specialists: Specialist[] = Array.isArray(parsed.specialists)
      ? parsed.specialists
          .filter((s: string) => VALID_SPECIALISTS.includes(s as Specialist))
          .slice(0, 2)
      : [];
    const complexity: Complexity = ["low", "med", "high"].includes(parsed.complexity)
      ? parsed.complexity
      : "med";
    return {
